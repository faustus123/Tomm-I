...
const int battery_pin = 15;    // select the input pin for the potentiometer
int analogInputs[16];
...
void setup() {
  ...
  Serial1.begin(115200); // TX1,RX1 on arduio used for high speed communication
//  Serial1.begin(9600); // TX1,RX1 on arduio used for high speed communication
...
//-------------------------------
void GetHCSR04State(String &str) {
  // This uses the traditional trig/echo method of reading
  // the ultrasonic sensor. This is better than the I2C
  // method (see below) because it lets us control the 
  // timeout.
  // Set trig_pin HIGH state for 10 micro seconds
  digitalWrite(trig_pin, HIGH);
  delayMicroseconds(10);
  digitalWrite(trig_pin, LOW);
  // Read the echo_pin, returns the sound wave travel time in microseconds
  float duration_us = pulseIn(echo_pin, HIGH, 3000);
  last_distance_cm = duration_us * 0.034 / 2;  // /2 is for round trip

  if( last_distance_cm > 400.0 ) last_distance_cm = -1;
  str = String(", \"dist_front\":") + last_distance_cm;

  digitalWrite(trig_pin, LOW); // clear trig pin in preparation for next cycle
}

//-------------------------------
// GetHCSR04StateI2C
//-------------------------------
void GetHCSR04StateI2C(String &str) {
